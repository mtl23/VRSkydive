	...
	public class Layout : Editor
	{
		...
		#region Draw
			...
			public void Label (string label, string tooltip="", float width=1f, bool prefix=false, bool bold=false, TextAnchor align=TextAnchor.LowerLeft, string url=null)
			{
				...
				//setting style
				labelStyle.normal.textColor = url!=null ? new Color(0.3f, 0.5f, 1f) : fieldStyle.normal.textColor;
			...
			public bool Field<T> (ref T param, bool allowSceneObject=false, float width=1f)
			{
				...
				bool curChange = false;
				if (typeof(T) == typeof(float))
					if (Mathf.Approximately((float)(object)param, (float)(object)newParam))  { change = true; curChange = true; }
				else if (typeof(T) == typeof(Color))
				{
					//Color oldCol = (Color)(object)param; Color newCol = (Color)(object)newParam;
					//if (Mathf.Approximately(oldCol.r,newCol.r) && Mathf.Approximately(oldCol.g,newCol.g) && Mathf.Approximately(oldCol.b,newCol.b) && Mathf.Approximately(oldCol.a,newCol.a))
					//	{ change = true; curChange = true; }
					
				}
				else
					if (!EqualityComparer<T>.Default.Equals(param, newParam)) { change = true; curChange = true; }
				
				change = false; curChange = false;
